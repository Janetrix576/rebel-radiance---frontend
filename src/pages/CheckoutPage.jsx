        ...
        if (paymentMethod === 'M-Pesa') {
            try {
                ...
                if (!checkout_request_id) {
                    throw new Error('CheckoutRequestID not found in response.');
                }
            ...
            return (
                <div className="flex items-center justify-center py-16">
                    <div className="text-center p-8 bg-white shadow-lg rounded-xl">
                        ...
                        <button onClick={() => navigate("/products")} className="px-6 py-2 bg-indigo-600 text-white font-semibold rounded-lg">Continue Shopping</button>
                    </div>
                </div>
            );
        }
                ...
                return (
                    <div className="flex items-center justify-center p-4">
                        <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-lg">
                            <h1 className="text-3xl font-bold text-center mb-6 text-indigo-600">Shipping Details</h1>
                            ...
                            <div className="border-t border-gray-200 pt-4 mb-6 space-y-2 text-gray-600">
                                ...
                                {cartItems.map(item => <div key={item.id} className="flex justify-between text-sm"><span>{item.name} (x{item.quantity})</span><span>Ksh {(item.price * item.quantity).toFixed(2)}</span></div>)}
                                <div className="flex justify-between border-t pt-2 mt-2"><span>Subtotal:</span><span>Ksh {subtotal.toFixed(2)}</span></div>
                                <div className="flex justify-between"><span>Shipping:</span><span>Ksh {shippingCost.toFixed(2)}</span></div>
                                <div className="flex justify-between font-bold text-lg text-gray-900 border-t pt-2 mt-2"><span>Total:</span><span>Ksh {total.toFixed(2)}</span></div>
                            </div>
                ...
                return (
                    <div className="flex items-center justify-center p-4">
                        <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl p-8">
                            ...
                            <div className="border-t pt-4 mb-4 font-bold text-lg">
                                <div className="flex justify-between"><span>Total:</span><span>Ksh {orderInfo.total.toFixed(2)}</span></div>
                            </div>
                            <button onClick={handleConfirmPayment} disabled={!paymentMethod} className={`w-full py-3 rounded-lg text-white font-bold text-lg transition ${paymentMethod ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-gray-400 cursor-not-allowed'}`}>
                                Confirm & Pay
                            </button>
                            <button onClick={() => setStep('checkout')} className="w-full text-indigo-600 hover:text-indigo-800 py-2 mt-2 transition">
            ...
            case 'processing':
                return (
                    <div className="flex flex-col items-center justify-center py-16 text-indigo-600">
                        <svg className="w-16 h-16 mb-4 animate-spin" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
                        <h2 className="text-2xl font-bold">Processing Payment...</h2>
                        <p className="mt-2 text-gray-600">Please check your phone for a prompt.</p>
                    </div>
                );
            case 'receipt':
                return (
                    <div className="flex flex-col items-center justify-center p-4">
                        <div className="max-w-md bg-white p-8 rounded-lg shadow-lg w-full text-center">
                            ...
                            <button onClick={() => navigate('/my-orders')} className="w-full mt-6 bg-indigo-600 text-white py-3 rounded-lg hover:bg-indigo-700 transition font-bold">
                                View My Orders
                            </button>
                        </div>
                    </div>

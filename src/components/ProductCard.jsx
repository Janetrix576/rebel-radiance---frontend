      ...
      <div className="relative w-full h-80">
        ...
        >
          {isFavorite(product.id) ? (
            <FaHeart size={20} className="text-electric-purple" />
          ) : (
            <FaRegHeart size={20} />
          )}
        </button>
      </div>
      <div className="p-4 bg-dark-gray flex-grow flex flex-col">
        <div className="text-center">
          <p className="text-xs text-electric-purple uppercase tracking-wider font-semibold mb-1">
            {categoryName} {subCategoryName && <span>/ {subCategoryName}</span>}
          </p>
          <h3 className="text-lg font-semibold text-white truncate mb-2">{product.name}</h3>
          <p className="text-md font-bold text-electric-blue">
            Ksh {product.display_price || 'N/A'}
          </p>
        </div>
      </div>
    </motion.div>

...
function CheckoutPage() {
  ...
  const [formData, setFormData] = useState({
    firstName: '', secondName: '', phone: '', selectedLocation: ''
  });
  ...
  const handleConfirmPayment = async () => {
    if (paymentMethod === 'M-Pesa') {
      try {
        ...
        if (!stkResponse.ok) throw new Error('STK Push initiation failed.');
        const stkData = await stkResponse.json();
        const { checkout_request_id } = stkData;
        if (!checkout_request_id) throw new Error('CheckoutRequestID not found in response.');

        const pollPaymentStatus = async () => {
          if (step !== 'processing') return;
          try {
            const res = await fetch(`https://rebel-radiance-backend.onrender.com/api/payment/status/${checkout_request_id}/`);
      ...
      window.FlutterwaveCheckout({
        ...
        customizations: { title: "Rebel Radiance", description: "Payment for merchandise" },
        callback: () => {
          closePaymentModal();
          setStep("receipt");
          clearCart();
    ...
    return (
      <div className="min-h-screen bg-dark-bg text-white p-6">
        <div className="text-center p-8 bg-gradient-to-br from-electric-purple via-deep-purple to-dark-bg border border-neon-blue shadow-lg rounded-xl">
          ...
          <button onClick={() => navigate("/products")} className="px-6 py-2 bg-electric-blue hover:bg-neon-blue text-white font-semibold rounded-lg">
            Continue Shopping
          </button>
        </div>
      </div>
        ...
        <div className="flex items-center justify-center p-4">
          <div className="bg-gradient-to-br from-electric-purple via-deep-purple to-dark-bg border border-neon-blue rounded-2xl shadow-2xl p-8 w-full max-w-lg">
            ...
            <div className="space-y-4 mb-6">
              <input name="firstName" className="w-full p-3 bg-dark-bg border border-neon-blue text-white placeholder-gray-400 rounded-lg focus:ring-2 focus:ring-electric-blue" type="text" placeholder="First Name*" value={formData.firstName} onChange={handleInputChange} />
              <input name="secondName" className="w-full p-3 bg-dark-bg border border-neon-blue text-white placeholder-gray-400 rounded-lg focus:ring-2 focus:ring-electric-blue" type="text" placeholder="Second Name*" value={formData.secondName} onChange={handleInputChange} />
              <input name="phone" className="w-full p-3 bg-dark-bg border border-neon-blue text-white placeholder-gray-400 rounded-lg focus:ring-2 focus:ring-electric-blue" type="tel" placeholder="Phone Number (e.g., 2547...)*" value={formData.phone} onChange={handleInputChange} />
            </div>
            <select name="selectedLocation" value={formData.selectedLocation} onChange={handleInputChange} className="w-full p-3 bg-dark-bg border border-neon-blue text-white placeholder-gray-400 rounded-lg mb-6">
              <option value="">-- Select Delivery Location --</option>
              {locations.map(loc => <option key={loc.name} value={loc.name}>{loc.name} - Ksh {loc.price.toFixed(2)}</option>)}
            </select>
            <div className="border-t border-gray-700 pt-4 mb-6 space-y-2 text-white">
              ...
              <div className="flex justify-between border-t border-gray-600 pt-2 mt-2"><span>Subtotal:</span><span>Ksh {subtotal.toFixed(2)}</span></div>
              <div className="flex justify-between"><span>Shipping:</span><span>Ksh {shippingCost.toFixed(2)}</span></div>
              <div className="flex justify-between font-bold text-lg text-white border-t border-gray-600 pt-2 mt-2"><span>Total:</span><span>Ksh {total.toFixed(2)}</span></div>
            </div>
            <button onClick={handleProceedToPayment} disabled={!isFormValid} className={`w-full py-3 rounded-lg text-white font-bold text-lg transition ${isFormValid ? 'bg-electric-blue hover:bg-neon-blue' : 'bg-gray-600 cursor-not-allowed'}`}>
              Proceed to Payment
            </button>
          </div>
        </div>
          ...
          <div className="w-full max-w-lg bg-gradient-to-br from-electric-purple via-deep-purple to-dark-bg border border-neon-blue rounded-2xl shadow-xl p-8">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div>
                ...
                <select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} className="w-full p-3 bg-dark-bg border border-neon-blue text-white rounded-lg">
            ...
            <div className="border-t pt-4 mb-4 font-bold text-lg text-white">
              <div className="flex justify-between"><span>Total:</span><span>Ksh {orderInfo.total.toFixed(2)}</span></div>
            </div>
            <button onClick={handleConfirmPayment} disabled={!paymentMethod} className={`w-full py-3 rounded-lg text-white font-bold text-lg transition ${paymentMethod ? 'bg-electric-blue hover:bg-neon-blue' : 'bg-gray-600 cursor-not-allowed'}`}>
              Confirm & Pay
            </button>
            <button onClick={() => setStep('checkout')} className="w-full text-electric-blue hover:text-neon-blue py-2 mt-2 transition">
      ...
      return (
        <div className="flex flex-col items-center justify-center p-4">
          <div className="max-w-md bg-gradient-to-br from-deep-purple to-dark-bg p-8 rounded-lg border border-neon-blue shadow-lg w-full text-center">
            ...
            <button onClick={() => navigate('/my-orders')} className="w-full mt-6 bg-electric-blue text-white py-3 rounded-lg hover:bg-neon-blue transition font-bold">
              View My Orders
            </button>
          </div>
        </div>
